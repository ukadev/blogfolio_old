            ...
            <div class="span12">
                <div class="widget-box">
                    <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
                        <h5>Posts</h5>
                        <span class="label label-success"><a class="white" href="{{route('postsCategoriesAddAdmin')}}" title="Add new Post Category"><i class="icon-plus"></i> Add new Post Category</a></span>
                    </div>
                        ...
                        <table class="table table-bordered data-table">
                            ...
                            @foreach($categories as $id => $category)
                                <tr class="{{ ($id % 2 == 0) ? "gradeX" : "gradeC" }}">
                                    <td>{{$category->name}}</td>
                                    <td> <span class="user-info">{{date('d/m/Y, h:i A', strtotime($category->created_at))}}</span></td>
                                    <td>
                                        <div class="text-right">
                                            <a class="btn btn-primary btn-mini" href="{{route('postsCategoriesEditAdmin', $category->id)}}"><i class="icon icon-edit"></i> Edit</a> | <a class="btn btn-danger btn-mini delPost" href="javascript:void(0)" data-post="{{$category->id}}" data-toggle="modal" data-target="#modalDel"><i class="icon icon-remove"></i> Delete</a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Are you sure you want to delete the selected post category?
                </div>
                <div class="modal-footer">
                    <form action="{{route('postsCategoriesDeleteAdmin', '')}}" class="form-horizontal" method="post" id="deletePostCategory">
