...
@section('content')
    ...
    <div id="content-header">
        <div id="breadcrumb"> <a href="{{route('panelAdmin')}}" title="Go to Admin Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="{{route('blogAdmin')}}" title="Go to Posts Page" class="tip-bottom">Posts</a> <a href="#" class="current">Edit Post</a> </div>
    </div>
    <!--End-breadcrumbs-->
    @if (count($errors) > 0)
                    ...
                    <div class="widget-content nopadding">
                        <form action="{{route('postsUpdateAdmin', $post->id)}}" class="form-horizontal" method="post">
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <input type="text" class="span11" placeholder="Title" value="{{$post->title}}" name="title" id="title" required>
                                </div>
                            </div>
                            <div class="control-group">
                                <label class="control-label">Slug:</label>
                                <div class="controls">
                                    <input type="text" class="span11" readonly name="slug" value="{{$post->slug}}" id="slug" />
                                </div>
                            </div>
                            <div class="control-group">
                                <label class="control-label">Content:</label>
                                <div class="controls">
                                    <textarea class="editor span11" name="content" id="content" title="content" required>{{$post->content}}</textarea>
                                </div>
                            </div>
